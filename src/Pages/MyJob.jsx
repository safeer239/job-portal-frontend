...
const MyJob = () => {
  ...
  useEffect(() => {
    setIsLoading(true)
    fetch('http://localhost:5000/my-job/mo@gmail.com').then(res=>res.json())
    ...
    <div className='max-w-screen-2xl container mx-auto xl:px-24 px-4'>
      <div>
        ...
        <div className='p-2 text-center mb-2'>
          ...
          <button onClick={handleSearch} className='bg-green text-white font-semibold px-8 py-2 rounded-sm mb-4'>Search</button>
        </div>
      </div>
            ...
            ( <tbody>
              {
                jobs.map((job,index)=>(
                  ...
                  <td className="border-t-0 px-6 align-middle border-l-0 border-r-0 text-xs whitespace-nowrap p-4 ">
                    ...
                    <button className='bg-blue text-white font-semibold px-8 py-2 rounded-sm mb-4 '>EDIT</button>
                    </Link>
    
                    <button onClick={()=>handleDelete(job._id)} className='bg-red-700 text-white font-semibold ml-3 px-8 py-2 rounded-sm mb-4'>DELETE</button>
                  </td>
                </tr>
                ))
              }
